...
public class ChessBoard {
	...
	/* separate objects have to be made for each piece because the same reference cannot point
	 to each one */
	...
	protected void fillEmptyPieceArray() {
		...
		// again a for loop cannot be used because each object has a different name
	...
	// just sets up the empty piece object for every corresponding square
	...
	protected void play(String s, int count) {
		switch (s.length()) { /* moves can be  length 2 ('e4'), length 3 ('Ke6'), length 4 or 5
		some lengths have multiple possibilities, which are all gone into */
		case 2:
			char a = s.charAt(0);
			int b = Character.getNumericValue(s.charAt(1)); 
			// above two statements convert the move into something workable
			outerloop: for (int i = 0; i < 8; i++) {
				for (int j = 0; j < 8; j++) {
					if (chessBoard[i][j].getPiece().getIdentity().equals("Pawn")
							...
							// above if statement evaluates where there exists a pawn that can move to that square
						// because it is a pawn, there will only be one that can move without capture
						if (count % 2 == 1) {
						...
						/* above two if statements just make sure that the right color pawn is moved depending on 
		...
		case 3:
			...
			switch (x) {
			case 'K':  // same logic as above but uses the's king's movement pattern and conditions
				// all the other pieces below follow the same, but with different movement conditions
				for (int i = 0; i < 8; i++) {
					for (int j = 0; j < 8; j++) {
			...
			case 'N':
				...
				break;
			case 'O': // this is King's side castling
				if (count % 2 == 1) {
					if (((King) getKing("white").getPiece()).getHasMoved() == false // makes sure king and rook haven't moved
							&& ((Rook) getChessBoardArray()[0][7].getPiece()).getHasMoved() == false) {
						if (getChessBoardArray()[0][5].getPiece().getIdentity().equals("Empty")
								&& getChessBoardArray()[0][6].getPiece().getIdentity().equals("Empty")) { // makes sure no square in between
							for (int i = 0; i < 8; i++) {
								for (int j = 0; j < 8; j++) {
									if (chessBoard[i][j].getPiece().getColor().equals("white") && !(chessBoard[i][j]
										...
										chessBoard[0][6].setPiece(whiteKing);  //  cannot use move Because king can only move one square
				...
				if (count % 2 == 0) { // same but for black
				...
				break;
			default: 
				System.out.println("Sorry, invalid move inner 3");  
				break; 
			}
			break; 
		...
		case 4:
			...
			if (second == 'x') { 
				if (Character.isLowerCase(first) == true) {// this refers to a pawn capture
					for (int i = 0; i < 8; i++) {
						for (int j = 0; j < 8; j++) {
							if (chessBoard[i][j].getPiece().getIdentity().equals("Pawn")
									&& chessBoard[i][j].getPiece().canCapture(getSquare(fourth, third))) {
									// because of the difference between how pawns capture and move, can capture
								// had to be used. 
								if (count % 2 == 1) {
									if (chessBoard[i][j].getPiece().getColor().equals("white")) {
					...
					switch (first) { // the rest of these are for other piece captures which 
			...
			} else if (second != 'x') { // refers to two of the same piece that could both possible
				// move to that square, only rooks and knights fit this category. 
				...
				case 'N':
					...
					break;
				default:
					System.out.println("Sorry, you entered an invalid move inner 4");
				}
			}
		...
		case 5:
			...
			case 'O':
				...
				break;
			default:
				System.out.println("Error, invalid move inner 5");
				break; 
			}
			break;
		default:
			System.out.println("Error, invalid move outer");
			break;
		}
	...
	// just initializes the chessboard 2d array
	...
	// makes sure all squares refer to the same chessBoard array
	...
	// same as above but for the pieces not the squares
	...
	// assigns the chess notation coordinates, as well as the numerical coordinates for operational purposes
	...
	// tests to see if a target square does not have a piece of the same color
	...
	// used to determine whether or not a king is in check, goes through for each opposing color
	//piece and sees if it can capture the square that the king is on
	...
	// used to determine whether or not white or black moves
	...
	protected boolean evaluateCheckmate(String color, ChessBoard board) {
		...
		if (kingInCheck(color, board) == true) {
			...
			for (int i = 0; i < 8; i++) {
				for (int j = 0; j < 8; j++) {
					if (chessBoard[i][j].getPiece().getColor().equals(color)) { /* tests to see
					if each piece of the same color can move and block the check */
						for (int a = 0; a < 8; a++) {
							for (int b = 0; b < 8; b++) {
								...
								if (chessBoard[i][j].getPiece().canMove(chessBoard[a][b]) == true) {
									...
									if (kingInCheck(color, temp) == false) {
										result = false;
									} /* above 8 lines of code test to see if that piece can move to any 
									square that eliminates check */
									this.getChessBoardArray()[tempPosition.getRowValue()][tempPosition.getColumnValue()]
											.setPiece(tempPiece);
									this.getChessBoardArray()[a][b].setEmptyPiece(); // just resets board
	...
	// used for testing and debugging purposes and to display the board to the user
